...
resource "aws_dynamodb_table" "terraform_state_lock" {
  name           = "mlops-stacks-102-nofs-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
...
resource "aws_iam_policy" "my-s3-read-policy" {
  name        = "mlops-stacks-102-nofs-terraform-state-manage"
  description = "S3 bucket access policy for managing terraform state for mlops-stacks-102-nofs"
...
resource "aws_iam_user" "s3_bucket_cicd_user" {
  name = "mlops-stacks-102-nofs-terraform-cicd-user"
}
...
resource "aws_s3_bucket" "cicd_terraform_state" {
  bucket = "mlops-stacks-102-nofs-cicd-setup-tfstate"
...
resource "aws_dynamodb_table" "cicd_terraform_state_lock" {
  name           = "mlops-stacks-102-nofs-cicd-setup-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
