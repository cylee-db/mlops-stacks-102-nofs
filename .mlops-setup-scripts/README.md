...
* Run `aws configure --profile mlops-stacks-102-nofs` to configure an AWS CLI profile, passing the access key ID and secret access key from the previous step
* Run `export AWS_PROFILE=mlops-stacks-102-nofs` to indicate to Terraform that it should use the newly-created AWS CLI profile
  to authenticate to AWS

### Configure Databricks auth
* Configure a Databricks CLI profile for your staging workspace by running
  ``databricks configure --token --profile "mlops-stacks-102-nofs-staging" --host https://e2-demo-field-eng.cloud.databricks.com``, 
...
* Configure a Databricks CLI for your prod workspace by running ``databricks configure --token --profile "mlops-stacks-102-nofs-prod" --host https://e2-demo-field-eng.cloud.databricks.com``
...
Ensure a group named `mlops-stacks-102-nofs-service-principals` exists in the staging and prod workspace, e.g.
...
Then, grant the `mlops-stacks-102-nofs-service-principals` group [token usage permissions](https://docs.databricks.com/administration-guide/access-control/tokens.html#manage-token-permissions-using-the-admin-console)
